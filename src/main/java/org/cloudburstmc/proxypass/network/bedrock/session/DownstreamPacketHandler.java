...
public class DownstreamPacketHandler implements BedrockPacketHandler {
    ...
    private void dumpCreativeItems(ItemData[] contents) {
        List<LegacyCreativeItemEntry> entries = new ArrayList<>();
        for (ItemData data : contents) {
            ...
            entries.add(new LegacyCreativeItemEntry(id, damage, blockRuntimeId, blockTag, tagData));
        }

        LegacyCreativeItems items = new LegacyCreativeItems(entries);
    ...
    @Value
    @JsonInclude(JsonInclude.Include.NON_NULL)
    private static class LegacyCreativeItemEntry {
        String id;
        Integer damage;
    ...
    @Value
    private static class LegacyCreativeItems {
